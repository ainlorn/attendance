...
class LessonServiceImpl(
    ...
    override fun getLessonsInPeriod(groupId: UUID, startTime: OffsetDateTime, endTime: OffsetDateTime): List<LessonDto> {
    ...
    override fun scheduleLessonsBatch(groupId: UUID, request: ScheduleBatchRequest): List<LessonDto> {
        ...
        while (currentWeekStart < periodEnd) {
            for (it in request.scheduledLessons!!) {
                val startTime = currentWeekStart.plusDays(it.day!! - 1L).plusSeconds(it.startTime!!.toSecondOfDay().toLong())
                val endTime = currentWeekStart.plusDays(it.day!! - 1L).plusSeconds(it.endTime!!.toSecondOfDay().toLong())
                if (endTime <= startTime) throw IncorrectTimeRangeException()
